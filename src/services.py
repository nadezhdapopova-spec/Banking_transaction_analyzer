...
def get_transactions_list() -> list[dict]:
    ...
    try:
        ...
        setup_logger().error(f"Ошибка преобразования XLSX-файла {filepath} в Python объект: {e}.")
...
def get_profitable_cashback_categories(data: list[dict], month: int, year: int) -> str:
    ...
    try:
        ...
        setup_logger().info("Получены данные о кэшбэке")
        ...
        setup_logger().info("Данные о кэшбэке преобразованы в JSON-строку")
...
def make_simple_search(search_str: str) -> str:
    ...
    setup_logger().info(f"Транзакции отфильтрованы по запросу {search_str}")
    ...
    setup_logger().info("Данные о транзакциях преобразованы в JSON-строку")
...
def search_for_transfers_to_individuals(data: list[dict]) -> str:
    ...
    try:
        ...
        setup_logger().info("Получены транзакции - переводы физлицам")
        ...
        setup_logger().info("Данные о переводах физлицам преобразованы в JSON-строку")
