...
def get_information_home_page(date_str: str,
    ...
    setup_logger().info("Python-объект преобразован в JSON-строку для страницы 'Главная'")
...
def get_events_information(date_str: str,
    ...
    setup_logger().info("Python-объект преобразован в JSON-строку для страницы 'События'")
...
def filter_transactions(transactions_df: pd.DataFrame,
    ...
    try:
        ...
        setup_logger().info("Транзакции отфильтрованы по дате")
        return filtered_transactions

    except KeyError as e:
        setup_logger().error(f"Ошибка фильтрации транзакций по дате: {e}")
...
def get_card_spent_cashback(transactions_df: pd.DataFrame) -> list[dict]:
    ...
    try:
        ...
        setup_logger().info("Получены сумма расходов, кешбэк по каждой карте")
        ...
        setup_logger().error(f"Ошибка при получении данных по каждой карте: {e}")
        raise KeyError(f"Ошибка: {e}")
...
def get_top_five_transactions(transactions_df: pd.DataFrame) -> list[dict]:
    ...
    try:
        ...
        setup_logger().info("Транзакции отсортированы по сумме платежа")
        ...
        setup_logger().error(f"Ошибка при сортировке транзакций по сумме платежа: {e}")
        raise KeyError(f"Ошибка: {e}")
...
def get_total_expenses(transactions_df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        setup_logger().info("Получена сумма расходов")
        ...
        setup_logger().error(f"Ошибка при получении суммы расходов: {e}")
        raise KeyError(f"Ошибка: {e}")
...
def get_total_income(transactions_df: pd.DataFrame) -> dict:
    ...
    try:
        total_amount = transactions_df[transactions_df["Сумма операции"] > 0]["Сумма операции"].sum()
        setup_logger().info("Получена сумма поступлений")
        ...
        setup_logger().error(f"Ошибка при получении суммы поступлений: {e}")
        raise KeyError(f"Ошибка: {e}")
...
def get_top_categories_expenses(transactions_df: pd.DataFrame) -> list[dict]:
    ...
    try:
        ...
        setup_logger().info("Получена сумма расходов по категориям")
        ...
        setup_logger().error(f"Ошибка при получении суммы расходов по категориям: {e}")
        raise KeyError(f"Ошибка: {e}")
...
def get_top_categories_income(transactions_df: pd.DataFrame) -> list[dict]:
    ...
    try:
        ...
        setup_logger().info("Получена сумма поступлений по категориям")
        ...
        setup_logger().error(f"Ошибка при получении суммы поступлений по категориям: {e}")
        raise KeyError(f"Ошибка: {e}")
...
def get_transfers_and_cash_expenses(transactions_df: pd.DataFrame) -> list[dict]:
    ...
    try:
        ...
        setup_logger().info("Получена сумма расходов по категориям 'Наличные' и 'Переводы'")
        ...
        setup_logger().error(f"Ошибка при получении данных по категориям 'Наличные' и 'Переводы': {e}")
        raise KeyError(f"Ошибка: {e}")
